...
> 5、避免使用with表达式，因为它增加作用域链的长度(with可以将一个没有或有多个属性的对象处理一个完全隔离的词法作用域，同时定义在这个with块内部var声明会被添加早with所在的函数作用域中)，with通常被当做重复引用同一个对象的多个属性的快捷方式，可以不需要重复引用对象本身。而且应当小心的对待try-catch的catch子句，它具有同样效果。
