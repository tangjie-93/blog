...
+ 4、`diff`过程整体遵循**深度优先、同层比较**的策略；两个节点之间的比较会根据它们是否拥有子节点或者文本节点做不同的操作；在比较两组子节点时，首先假设首位节点可能相同做4次对比尝试(**首首比较，尾尾比较，首尾比较，尾兽比较**——`updateChildren`)，如果没有找到相同节点才按照通用方式遍历查找，查找结束再按情况处理剩下的节点；借助 `key` 通常可以很快速的找到相同节点，因此整个patch过程非常高效。(**高效性**)